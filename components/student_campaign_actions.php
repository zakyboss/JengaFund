...
try {
    ...
    if ($action === 'confirm_payment') {
        ...
        if (($milestone['status'] ?? '') !== 'disbursed' || empty($milestone['disbursed_at'])) {
            throw new RuntimeException('You can only confirm receipt after admin marks this payout as paid.');
        }

        $confirmations = loadPaymentConfirmationsForMilestones($pdo, [$milestone], $studentId);
        if (isset($confirmations[$milestoneId])) {
            throw new RuntimeException('You have already confirmed this payment.');
        }

        $amount = (float) ($milestone['disbursed_amount'] ?? 0);
        if ($amount <= 0) {
            $amount = round((float) $campaign['funds_raised'] * ((float) $milestone['disbursement_percent'] / 100), 2);
        }

        $studentName = $_SESSION['user_name'] ?? 'Student';
        $studentMsg = milestonePaymentConfirmStudentMessage($amount, $milestone['title'], $campaign['title']);
        $adminMsg = milestonePaymentConfirmAdminMessage($studentName, $amount, $milestone['title'], $campaign['title']);

        notifyUser($pdo, $studentId, 'project_update', $studentMsg, $milestoneId);
        notifyAllAdmins($pdo, 'project_update', $adminMsg, $milestoneId);

        $_SESSION['success_message'] = 'Thank you. Your payment confirmation has been recorded.';
        header('Location: ' . $redirect);
        exit;
    }
